import { createHermesId } from "./ids.js";
import type { ExternalRef, ExternalSystem, SyncDirection } from "./types.js";
...
export class ExternalRefRepository {
  ...
  private fromRow(row: Record<string, unknown>): ExternalRef {
    return {
      id: row.id as string,
      hermesObjectType: row.hermes_object_type as ExternalRef["hermesObjectType"],
      hermesObjectId: row.hermes_object_id as string,
      system: row.system as ExternalRef["system"],
      externalId: row.external_id as string,
      externalUrl: (row.external_url as string | null) ?? null,
      syncDirection: row.sync_direction as ExternalRef["syncDirection"],
